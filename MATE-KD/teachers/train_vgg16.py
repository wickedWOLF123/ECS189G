...
def create_vgg16_cifar10():
    """
    Create VGG-16 adapted for CIFAR-10
    """
...
def main():
    parser = argparse.ArgumentParser(description='Train VGG-16 on CIFAR-10')
    ...
    # Create model
    model = create_vgg16_cifar10().to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            save_checkpoint(model, optimizer, epoch, test_acc, 'vgg16_best', 'checkpoints')
        
        if (epoch + 1) % args.save_freq == 0:
            save_checkpoint(model, optimizer, epoch, test_acc, f'vgg16', 'checkpoints')
